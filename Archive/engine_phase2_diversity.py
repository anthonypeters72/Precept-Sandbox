...
def ensure_min_precepts(
    *,
    ...
    def _corpus_of_ref(ref: str) -> str:
        ...
        if book == "quran":
            return "quran"
        # treat apoc books as apoc bucket (expand later as needed)
        if book in {
...
# --- Rerank by rarity-weighted overlap (minimal relevance upgrade) ---
        ...
        if scored:
            ...
            # --- Phase 2: corpus diversity balancing (penalize overrepresented corpora/books) ---
            used_buckets = {_bucket(r) for r in final_precepts}
            corpus_counts: dict[str, int] = {}
            for rr in final_precepts:
                c0 = _corpus_of_ref(rr)
                corpus_counts[c0] = corpus_counts.get(c0, 0) + 1

            CORPUS_PEN = 0.35     # tune later
            BUCKET_PEN = 0.15     # tune later (discourages same-book pileups)

            diversified = []
            for s, r, top_terms, mcount, qcount in scored:
                c = _corpus_of_ref(r)
                b = _bucket(r)
                adj = float(s) - (CORPUS_PEN * float(corpus_counts.get(c, 0))) - (BUCKET_PEN if b in used_buckets else 0.0)
                diversified.append((adj, float(s), r, top_terms, mcount, qcount, c, b))

            diversified.sort(key=lambda x: (x[0], x[1]), reverse=True)
            sim_refs = [r for _, _, r, *_ in diversified]
