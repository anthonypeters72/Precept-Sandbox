...
class Engine:
    ...
    idf_df: dict[str, int] | None = None
    idf_n_docs: int | None = None
    idf_oov: float | None = None
...
def build_global_idf_from_all_verses(all_verses: list[tuple[str, str, str]]) -> tuple[dict[str, float], dict[str, int], int, float]:
    """
    True global IDF over the entire loaded corpus at the verse-document level.

    all_verses items are (ref, text, corpus_name).
    IDF = log((N+1)/(df+1)) + 1, with an explicit __OOV__ default.
    """
    df: dict[str, int] = {}
    N = 0

    for ref, txt, corpus_name in (all_verses or []):
        if not txt or not isinstance(txt, str):
            continue
        toks = set(meaningful_tokens(txt))
        if not toks:
            continue
        N += 1
        for t in toks:
...
def weighted_overlap_score(query_text: str, cand_text: str, idf: dict[str, float]) -> tuple[float, list[tuple[str, float]], int, int]:
    ...
    for t in matched_uniq:
        w = float(idf.get(t, idf.get("__OOV__", 1.0)))
...
def lookup_text_any(corpora: dict, ref: str) -> tuple[str | None, str | None]:
    ...
    # If user asks Letter of Jeremiah 1:x, map to 6:x (common numbering)
    ...
    enoch = corpora.get("enoch")
    hermas = corpora.get("hermas")
    ...
    if low.startswith("quran"):
        ...
        for qref in quran_ref_variants(ref_n):
            # If Quran is string-keyed, this hits
            hit = lookup_text_flexible(quran, qref)
            if hit:
                return hit, "quran"

            # If Quran is tuple-keyed, this hits
    ...
    # --- Jasher: tuple-keyed (keep your existing parse) ---
    ...
    # --- Tuple lookup priority order ---
    for name, corpus in (("apoc", apoc), ("enoch", enoch), ("hermas", hermas), ("kjv", kjv)):
...
def build_engine(
    *,
    ...
    # --- registry corpora (enoch/hermas/etc) ---
    ...
    # --- build TRUE global IDF (verse-documents) ---
    print("[DEBUG] Building global IDF (verse-documents)...") if DEBUG else None

    global_idf, global_df, global_N, global_oov = build_global_idf_from_all_verses(all_verses)

    if DEBUG:
        print(f"[DEBUG] Global IDF docs={global_N} vocab={len(global_idf)} oov={global_oov:.3f}")
